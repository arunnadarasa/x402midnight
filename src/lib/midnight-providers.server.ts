...
async function resolveContractModule(zkPath: string) {
  ...
  throw new Error(`Compiled contract missing under ${zkPath}/contract`);
}

let cachedAddress: string | null = null;
let ctxPromise: Promise<UndeployedCtx> | null = null;

export async function getUndeployedCtx(address: string): Promise<UndeployedCtx> {
...
async function buildCtx(address: string): Promise<UndeployedCtx> {
  ...
  const zkConfigPath = resolve(process.cwd(), `contracts/managed/${CONTRACT_KEY}`);
  const buyerSecret = Uint8Array.from(Buffer.from(GENESIS_SEED, "hex"));
...
export { PRIVATE_STATE_ID, PRIVATE_STATE_STORE, GENESIS_SEED };
